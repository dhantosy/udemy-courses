...
describe('Clue', () => {
  ...
  it('initializes the reveal state to be false', () => {
    expect(clueWrapper.state().isRevealed).toBe(false)
  })
  ...
  describe('when clicking on the clue', () => {
    ...
    it('sets the isRevealed state to be true', () => {
      expect(clueWrapper.state().isRevealed).toBe(true)
    })

    it('sets the answer with text-revealed class', () => {
      expect(clueWrapper.find('h5').at(1).hasClass('text-revealed')).toBe(false)
    })
  })
})
